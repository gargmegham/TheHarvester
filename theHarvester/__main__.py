...
async def start(rest_args: Optional[argparse.Namespace] = None):
    ...
    parser.add_argument(
        "-b",
        "--source",
        help="""anubis, baidu, bevigil, binaryedge, bing, bingapi, bufferoverun, censys, certspotter, crtsh,
    ...
    async def store(
        ...
        """
        await search_engine.process(
            use_proxy
        ) if process_param is None else await search_engine.process(
            process_param, use_proxy
        )
        ...
        if set(engines).issubset(Core.get_supportedengines()):
            ...
            for engineitem in engines:
                if engineitem == "anubis":
                    ...
                    stor_lst.append(
                        store(
                            qwant_search, engineitem, store_host=True, store_emails=True
                        )
                    )
                    ...
                    try:
                        ...
                        stor_lst.append(
                            store(sublist3r_search, engineitem, store_host=True)
                        )
                    except Exception as e:
                        print(e)
                    ...
                    stor_lst.append(
                        store(
                            yahoo_search, engineitem, store_host=True, store_emails=True
                        )
                    )
    ...
    if len(args.screenshot) > 0:
        ...
        if path_exists:
            ...
            unique_resolved_domains = {
                url.split(":")[0] for url in full if ":" in url and "www." not in url
            }
